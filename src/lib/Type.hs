...
type TypeCheckEnv i o = (RenameEnv i o, OptionalEnv (TypeEnv o, EffectRow o))
type TypeM i o = ReaderT (TypeCheckEnv i o) Except

class (HasNames e, PrettyH e) => HasType e where
  typeCheck :: e i -> TypeM i o (Type o)

getType :: (HasCallStack, HasType e) => e n -> Type n
getType x = ignoreExcept $ ctx $ runTypeCheck (id, SkipChecks) $ typeCheck x
...
checkType env eff x = void $ ctx $ runTypeCheck tyCheckEnv $ typeCheck x
  where
    tyCheckEnv = (id, CheckWith (env, eff))
...
-- -- For debugging
-- traceCheckM :: (HasCallStack, HasType e, Monad m) => e n -> m ()
-- traceCheckM x = traceCheck x (return ())

-- traceCheck :: (HasCallStack, HasType e) => e n -> a -> a
-- traceCheck x y =
...
exprEffs expr = case expr of
  ...
  Op op   -> case op of
    PrimEffect ref m -> case m of
      ...
      where RefTy (Var (AnnVar h _)) _ = getType ref
...
instance CoreVariant (Atom n) where
  checkVariant atom = addExpr atom $ case atom of
    Var (AnnVar _ ty) -> checkVariant ty
    ...
    BoxedRef _ _ _   -> neverAllowed  -- only used internally in Imp lowering
    ProjectElt _ (AnnVar _ ty) -> checkVariant ty
...
lowerNamesErr :: (MonadError Err m, BindsNames b, HasNames e)
              => AnnBinderP b ann n l -> e l -> m (e n)
...
-- typeReduceBlock bindings (Block decls result) = do
--   let localBindings = fromRecEnvFrag $ asBindingsFrag decls
--   let newBindings = RecEnvFrag (fromRecEnvFrag bindings >>> localBindings)
--   ans <- typeReduceExpr newBindings result
--   lowerNames (asScopeFrag bindings) localBindings ans
...
typeReduceExpr :: Bindings n -> Expr n -> Maybe (Atom n)
typeReduceExpr bindings expr = case expr of
  Atom val -> return $ typeReduceAtom bindings val
  App f x -> do
    let f' = typeReduceAtom bindings f
    let x' = typeReduceAtom bindings x
    ...
    case f' of
      Lam (Abs b (WithArrow arr block)) | arr == PureArrow || arr == ImplicitArrow ->
        typeReduceBlock bindings $ applySubst (asScopeFrag bindings) (b@>SubstVal x') block
