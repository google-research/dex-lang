...
module Syntax (
    ...
    WithSrc (..), WithSrcE (..), WithSrcB (..), AnnVarP (..),
...
data AnnVarP (ann::E) n = AnnVar (Name n) (ann n)  deriving (Show, Eq)
...
data UDataDef (n::S) where
  UDataDef :: UAnnBinderList n l -> ListE (EmptyNest UAnnBinder) l -> UDataDef n
...
data UDecl (n::S) (l::S) where
  ...
  UInterface
    ...
    ->   [UType (n:-:p)]          -- superclasses
    ->   [UType (n:-:p)]          -- method types
    -> PlainBinder n l          -- class name
    ->   PlainBinderList (n:-:l) l'   -- method names
    ->     UDecl n (l:-:l')
  UInstance
    :: Nest UPatAnnArrow n l  -- dictionary args (i.e. conditions)
    ->   UType (n:-:l)        -- dictionary type
    ->   [UMethodDef (n:-:l)]       -- method definitions
...
type UPatAnn = AnnBinderP UPat (MaybeE UType)
type UPatAnnArrow = AnnBinderP UPatAnn (LiftE UArrow)

type UAnnBinder     = AnnBinderP     PlainBinder UType :: B
type UAnnBinderList = AnnBinderListP PlainBinder UType :: B
...
data UPat' (n::S) (l::S) where
  UPatUnit   :: UPat' n EmptyScope
  UPatBinder :: PlainBinder n l         -> UPat' n l
  UPatPair   :: UPat n n' -> UPat (n:-:n') l  -> UPat' n (n':-:l)
...
pattern UnderscoreUPat = WithSrcB Nothing (UPatBinder Ignore)
...
data WithSrcE (e :: E) n   = WithSrcE  SrcCtx (e n)   deriving Show
data WithSrcB (b :: B) n l = WithSrcB SrcCtx (b n l) deriving Show

srcPos :: WithSrc a -> SrcCtx
srcPos (WithSrc pos _) = pos

srcPosH :: WithSrcE e n -> SrcCtx
srcPosH (WithSrcE pos _) = pos
...
outputStreamPtrName :: Name SourceScope
...
type SourceModule = UDecl SourceScope SourceScope
...
data SourceBlock' = RunModule SourceModule
                  | Command CmdName (Name SourceScope, SourceModule)
                  | GetNameType (Name SourceScope)
...
type IBinderList = AnnBinderP PlainBinderList (ListE ITypeH)
type IVar = AnnVarP ITypeH
type IType = BaseType
type Size = IExpr

type IFunVar = AnnVarP (LiftE IFunType)
data IFunType = IFunType CallingConvention [IType] [IType] -- args, results
                deriving (Show)

type ITypeH    = LiftE IType
...
class HasUVars (e::E) where
  freeUVars :: e n -> NameSet n

-- -- class BindsUVars (b :: * -> * -> *) where
-- --   boundUVars :: b n l -> BinderListP UnitE n l
...
emptyBindings :: Bindings EmptyScope
emptyBindings = RecEnvFrag emptyEnv
...
class BindsNames b => BindsNamesCore (b::B) where
...
traverseNamesFromSubst :: (Subst e, Monad m)
                           => Scope o -> NameTraversal m i o -> e i -> m (e o)
traverseNamesFromSubst scope (NameTraversal f renamer) e =
  traverseNamesCore scope (CoreNameTraversal f' renamer) e
  where f' name = Rename <$> f name

traverseNamesBindersFromBindsNamesCore
  :: (BindsNamesCore b, Monad m)
  => Scope o -> NameTraversal m i o -> b i l -> m (Fresh b o l)
traverseNamesBindersFromBindsNamesCore _ _ _ = undefined
...
instance HasNames EffectRow where traverseNames = traverseNamesFromSubst
instance Subst EffectRow where
  traverseNamesCore = undefined

instance HasNames Atom where traverseNames = traverseNamesFromSubst
instance Subst    Atom where
  traverseNamesCore = undefined
...
instance HasNames Expr where traverseNames = traverseNamesFromSubst
instance AlphaEq  Expr where
  alphaEq = undefined
instance Subst    Expr where
  traverseNamesCore = undefined

instance HasNames Block where traverseNames = traverseNamesFromSubst
...
instance (Subst e) => HasNames (WithArrow e) where
  traverseNames = traverseNamesFromSubst
...
instance BindsNames Decl where
  traverseNamesBinders = traverseNamesBindersFromBindsNamesCore
...
tn :: HasNames e => Monad m => Scope o -> NameTraversal m i o -> e i -> m (e o)
tn = traverseNames

-- shorthand for instances
tnb :: BindsNames b => Monad m => Scope o -> NameTraversal m i o -> b i l -> m (Fresh b o l)
tnb = traverseNamesBinders
...
wtnb
  ...
  -> m a
wtnb = withTraverseNamesBinders

instance HasNames UExpr' where
  traverseNames s t expr = case expr of
    UVar v -> UVar <$> tn s t v
    ULam p arr e -> wtnb s t p \s' t' p' -> ULam p' arr <$> tn s' t' e
    UPi  p arr ty -> wtnb s t p \s' t' p' ->
      UPi p' <$> mapM (tn s' t') arr <*> tn s' t' ty
    UApp arr f x -> UApp arr <$> tn s t f <*> tn s t x
    UDecl d e -> wtnb s t d \s' t' d' -> UDecl d' <$> tn s' t' e
    UFor d p e -> wtnb s t p \s' t' p' -> UFor d p' <$> tn s' t' e
    UCase scrut alts -> UCase <$> tn s t scrut <*> mapM (tn s t) alts
    UHole -> return UHole
    UTypeAnn e ann-> UTypeAnn <$> tn s t e <*> tn s t ann
    UTabCon  xs -> UTabCon <$> mapM (tn s t) xs
    UIndexRange l h -> UIndexRange <$> mapM (tn s t) l <*> mapM (tn s t) h
    UPrimExpr e -> UPrimExpr <$> mapM (tn s t) e
    URecord _ -> undefined
...
instance BindsNames UDecl where
  traverseNamesBinders s t decl = case decl of
    ULet ann pat expr -> do
      expr' <- traverseNames s t expr
      tnb s t pat >>= \case
        Fresh ext pat' renamer ->
          return $ Fresh ext (ULet ann pat' expr') renamer
    UDataDefDecl dataDef tcBinder dcBinders -> do
      dataDef' <- tn s t dataDef
      tnb s t (   NestPair tcBinder  dcBinders) >>= \case
    ...
    UInterface params scTys methodTys classBinder methodBinders -> do
      Abs  params' (PairE (ListE scTys') (ListE methodTys')) <- tn s t $
       Abs params  (PairE (ListE scTys ) (ListE methodTys ))
      tnb s t (   NestPair classBinder  methodBinders ) >>= \case
    ...
    UInstance args dictTy methods b -> do
      Abs  args' (PairE dictTy' (ListE methods')) <- tn s t $
       Abs args  (PairE dictTy  (ListE methods ))
      tnb s t b >>= \case
        Fresh ext b' renamer ->
          return $ Fresh ext (UInstance args' dictTy' methods' b') renamer

  boundScope decl = case decl of
    ULet _ pat _ -> boundScope pat
    UDataDefDecl _ tcBinder dcBinders -> boundScope $ NestPair tcBinder dcBinders
    UInterface _ _ _ classBinder methodBinders -> boundScope $ NestPair classBinder  methodBinders
    UInstance _ _ _ b -> boundScope b

instance HasNames UAlt where
  traverseNames s t alt = undefined

instance HasNames UDataDef where
  traverseNames s t def = undefined

instance HasNames UMethodDef where
  traverseNames s t def = undefined

instance BindsNames UPat' where
  traverseNamesBinders s t pat = case pat of
    UPatUnit -> return $ Fresh emptyDisjoint UPatUnit emptyEnv
    UPatBinder b -> fmapFresh UPatBinder <$> tnb s t b
    -- UPatPair p1 p2 -> do
    --   pair <- tnb s t $ NestPair p1 p2
...
instance Show (UPat' n l) where
  show = undefined

instance IsString (UExpr' SourceScope) where
...
instance BindsNames b => BindsNames (WithSrcB b) where
  traverseNamesBinders scope t (WithSrcB ctx b) =
    fmapFresh (WithSrcB ctx) <$> traverseNamesBinders scope t b

  boundScope = undefined

instance HasNames e => HasNames (WithSrcE e) where
  traverseNames scope t (WithSrcE ctx e) =
    WithSrcE ctx <$> traverseNames scope t e
