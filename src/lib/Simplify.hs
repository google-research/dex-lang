...
module Simplify (simplifyTopFun, exprAsNullaryFun) where
...
exprAsNullaryFun :: CExpr VoidS -> CoreLamExpr VoidS
...
data SimpVal (n::S) =
 ...
 | SimpCCon (WithSubst CExpr n)  -- local non-data vars
...
--   PrimOp _ (RefOp ref (ProjRef UnwrapNewtype)) -> SimpAtom <$> toDataAtom ref
--   PrimOp ty op  -> do
--     ty' <- getRepType ty
...
simplifyLam :: CoreLamExpr i -> SimplifyM i o (LamExpr o)
simplifyLam (CoreLamExpr _ ab) = go ab
 where
   go :: Abs (Nest CBinder) CExpr i -> SimplifyM i o (LamExpr o)
