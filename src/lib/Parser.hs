...
sourceBlock' =
      proseBlock
  <|> topLevelCommand
  <|> liftM RunModule (topDecl <* eolf)
  <|> liftM RunModule (instanceDef True  <* eolf)
  <|> liftM RunModule (instanceDef False <* eolf)
  <|> liftM RunModule (interfaceDef <* eolf)
...
exprAsModule :: UExpr S -> (Name S, SourceModule)
exprAsModule e = (v, d)
  where
    v = fromRawSourceName "_ans_"
    d = ULet PlainLet (WithSrcH2 (srcPosH e) (nameToPat v) :> NothingH) e
...
topLet = do
  ...
  ~(ULet _ (p :> H ann) rhs) <- decl
  let (ann', rhs') = addImplicitImplicitArgs ann rhs
  return $ ULet lAnn (p :> H ann') rhs'
...
addImplicitImplicitArgs (Just typ) ex =
  ...
  where
    ...
    addImplicitArg v (ty, e) =
      ( ns $ UPi  (uPat :> NothingH) ImplicitArrow ty
      , ns $ ULam (uPat :> NothingH) ImplicitArrow e)
...
dataDef = do
  ...
  let def = UDataDef (uAnnBindersAsUAnnBinderList tyConParams) (H dataConstructors)
  return $ UDataDefDecl def tyConName $ bindersAsBinderList dataConNames

tyConDef :: Parser (PlainBinder S S, [UAnnBinder S S])
tyConDef = do
  ...
  bs <- many $ label "type constructor parameter" $ do
    ...
    return $ SourceBinder v :> ty
  return (SourceBinder con, bs)
  where tyKind = ns $ UPrimExpr $ TCExpr TypeKind

-- TODO: dependent types
dataConDef :: Parser (PlainBinder S S, EmptyNest UAnnBinder S)
dataConDef = do
  dataConName <- upperName
  bs <- manyNested dataConDefBinder
  return (SourceBinder dataConName, Abs bs UnitH)

dataConDefBinder :: Parser (UAnnBinder S S)
dataConDefBinder = annBinder <|> ((Ignore:>) <$> containedExpr)

bindersAsBinderList ::[PlainBinder S S] -> PlainBinderList S S
bindersAsBinderList = toNest

uAnnBindersAsUAnnBinderList ::[UAnnBinder S S] -> UAnnBinderList S S
uAnnBindersAsUAnnBinderList annBinders = toNest binders :>> anns
  where (binders, anns) = unzip [(binder, ann) | binder :> ann <- annBinders]
...
instanceDef isNamed = do
  name <- case isNamed of
    False -> keyWord InstanceKW $> Ignore
    True  -> keyWord NamedInstanceKW *> (SourceBinder <$> anyName) <* sym ":"
  ...
  let argBinders =
        [WithSrcH2 Nothing (nameToPat v) :> NothingH :> LiftH ImplicitArrow | v <- implicitArgs] ++
        explicitArgs                                                       ++
        [UnderscoreUPat :> JustH c :> LiftH ClassArrow | c <- constraints]
  ...
  where
    ...
    addClassConstraint c ty = ns $ UPi (UnderscoreUPat :> JustH c) ClassArrow ty
...
uLamExpr = do
  ...
  arrowType <-
    ...
    <|> (arrow (return ()) >>= \case
          ...
          arr -> return arr)
  body <- blockOrExpr
  return $ buildLam (map (:>LiftH arrowType) bs) body

buildLam :: [UPatAnnArrow S S] -> UExpr S -> UExpr S
buildLam binders body@(WithSrcH pos _) = case binders of
  [] -> body
  -- TODO: join with source position of binders too
  (b:>LiftH arr):bs -> WithSrcH (joinPos pos' pos) $ ULam b arr $ buildLam bs body
...
uViewExpr = do
  ...
  argTerm
  body <- blockOrExpr
  return $ buildLam (zipWith (:>) bs (repeat (LiftH TabArrow))) body
...
uForExpr = do
  ...
  if trailingUnit
    then return $ ns $ UDecl (ULet PlainLet (UnderscoreUPat :> NothingH) e) $
                                ns unitExpr
    else return e

nameToPat :: Name S -> UPat' S S
nameToPat v = UPatBinder (SourceBinder v)
...
wrapUStatements statements = case statements of
  ...
  (s, pos):rest -> WithSrcH (Just pos) $ case s of
    ...
    Right e -> UDecl d $ wrapUStatements rest
      where d = ULet PlainLet (UnderscoreUPat :> NothingH) e
  ...
  where piBinderPat = do
          ...
          return $ case b of
            SourceBinder n :> a@(WithSrcH pos _) ->
              (WithSrcH2 pos (nameToPat n) :> JustH a)
            Ignore :> a -> (UnderscoreUPat :> JustH a)
            _ -> error "GHC bug? I think these patterns are complete"
...
globalEnumPat :: String -> UPat S S
globalEnumPat s = WithSrcH2 Nothing $ UPatCon (SourceName (fromString s)) Empty
...
leafPat =
  ...
  <|> (withSrcH2 $
          (UPatBinder <$>  (   (SourceBinder <$> lowerName)
                           <|> (underscore $> Ignore)))
...
patAnn = label "pattern" $ (:>) <$> pat <*> optionalH (annot containedExpr)
...
uDoSugar = withSrcH $ do
  keyWord DoKW
  body <- blockOrExpr
  return $ ULam (WithSrcH2 Nothing UPatUnit :> NothingH) (PlainArrow ()) body
...
infixArrow = do
  ...
  return \a b -> WithSrcH (Just pos) $ UPi (UnderscoreUPat :> JustH a) arr b

mkArrow :: Arrow S -> UExpr S -> UExpr S -> UExpr S
mkArrow arr a b = joinSrcH a b $ UPi (UnderscoreUPat :> JustH a) arr b

_withSrc :: Parser a -> Parser (WithSrc a)
_withSrc p = do
  (x, pos) <- withPos p
  return $ WithSrc (Just pos) x

optionalH :: Parser (e n) -> Parser (MaybeH e n)
optionalH p = H <$> optional p
...
_joinSrc :: WithSrc a -> WithSrc b -> c -> WithSrc c
_joinSrc (WithSrc p1 _) (WithSrc p2 _) x = WithSrc (joinPos p1 p2) x
...
mkName s = SourceName $ fromString s
