...
emitDecls (WithDecls decls result) = runSubstReaderT idSubst $ emitDecls' decls result
...
buildEffLam hint ty body = do
  withFreshBinder noHint (WithoutDecls $ TC HeapType) \h -> do
    hVar <- binderVar h
    let ty' = RefTy (Var hVar) (sink ty)
    withFreshBinder hint (WithoutDecls ty') \b -> do
      ref <- binderVar b
...
buildForAnn hint ann (IxType iTy ixDict) body = do
  lam <- withFreshBinder hint (WithoutDecls iTy) \b -> do
    v <- binderVar b
...
zeroAt ty = liftEmitBuilder $ go ty where
  ...
  go = \case
   ...
   TabTy (b:>ixTy) bodyTy -> do
     ixTy' <- emitDecls ixTy
     buildFor (getNameHint b) Fwd ixTy' \i -> do
       go =<< emitDecls =<< applySubst (b @> SubstVal (Var i)) bodyTy
   _ -> unreachable
...
maybeTangentType' ty = case ty of
  TabTy b (WithDecls ds bodyTy) -> do
    refreshAbs (Abs (PairB b ds) bodyTy) \(PairB b' ds') bodyTy' -> do
      bodyTanTy <- rec bodyTy'
      return $ TabTy b' $ WithDecls ds' bodyTanTy
  TC con    -> case con of
...
getUnpacked atom = do
  ...
  forM (enumerate positions) \(i, _) ->
    projectElt (ProjectProduct i) atom
{-# SCC getUnpacked #-}
...
projectTuple :: (Builder r m, Emits n, IRRep r) => Int -> Atom r n -> m n (Atom r n)
projectTuple i x = projectElt (ProjectProduct i) x

projectStruct :: EnvReader m => Int -> CAtom n -> m n (CAtom n)
projectStruct i x = do
  projs <- getStructProjections i (getType x)
  naryProjectElt projs x
{-# INLINE projectStruct #-}

naryProjectElt :: (Builder r m, Emits n) => [Projection] -> Atom r n -> m n (Atom r n)
naryProjectElt = undefined

projectElt :: (Builder r m, Emits n) => Projection -> Atom r n -> m n (Atom r n)
projectElt = undefined
...
reduceE monoid xs = liftEmitBuilder do
  ...
  a' <- emitDecls $ ignoreHoistFailure $ hoist n a
  ty' <- emitDecls ty
...
catMaybesE maybes = do
  TabTy n (WithDecls ds (MaybeTy a)) <- return $ getType maybes
  let tabTy = TabTy n $ WithDecls ds a
  justs <- liftEmitBuilder $ mapE isJustE maybes
  monoid <- andMonoid
  allJust <- reduceE monoid justs
  liftEmitBuilder $ emitIf allJust (MaybeTy tabTy)
    (JustAtom (sink tabTy) <$> mapE fromJustE (sink maybes))
    (return (NothingAtom $ sink tabTy))
...
-- === querying types of partially formed expressions ===
...
getReferentTy (Abs (PairB hB refB) UnitE) = undefined
...
getMethodIndex className methodSourceName = undefined
...
getDataConNameType dataCon = undefined
...
liftIFunType (IFunType _ argTys resultTys) = undefined
...
binderVar :: (Builder r m, Emits l, IRRep r) => Binder r n l -> m l (AtomVar r l)
binderVar = undefined
