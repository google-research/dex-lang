...
toAtomVar v = do
  ty <- undefined -- getType <$> lookupAtomName v
  return $ AtomVar v ty
...
instance (IRRep r, SubstE AtomSubstVal e) => SubstE AtomSubstVal (WithDecls r e)

-- === default visitor helpers based on substitution ===
...
visitBinders
  ...
  -> m i o a
visitBinders x y = undefined -- this will need a way to handle decls/expressions
-- Empty cont = getDistinct >>= \Distinct -> cont Empty
