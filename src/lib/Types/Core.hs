...
instance ToBindersAbs (Abs (Binders r) body) body r where
  toAbs = id
...
pattern UnaryLamExpr b body <- (asUnaryLamExpr -> Just (Abs b body))
  where UnaryLamExpr b body = LamExpr (UnaryNest (BD b Empty)) body
