...
lowerFullySequential :: (TopBuilder m, EnvReader m, Mut n) => Block n -> m n (DestBlock n)
lowerFullySequential b = liftM fst $ liftGenericTraverserMWithEmissions LFS do
...
data LFS (n::S) = LFS
type LowerM = GenericTraverserM TopEmissionsAllowed LFS
...
instance GenericTraverser TopEmissionsAllowed LFS where
...
traverseTabCon maybeDest tabTy elems = do
  ...
  Abs bord ufoBlock <- buildAbs noHint IdxRepTy \ord -> do
    -- TODO: this is the one place where we call back into `Simplify` from a
    -- downstream pass. Should we remove it?
