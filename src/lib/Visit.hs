...
visitAlt (Abs b body) = do
  visitGeneric (LamExpr (UnaryNest (BD b Empty)) body) >>= \case
    LamExpr (UnaryNest (BD b' Empty)) body' -> return $ Abs b' body'
    _ -> error "not an alt"
...
-- XXX: This doesn't handle the `Var`, `SimpInCore` cases. These
...
-- XXX: This doesn't handle the `TyVar` case. These should be
...
instance IRRep r => VisitGeneric (Expr r) r where
  visitGeneric = \case
    ...
    SynthDict ctx ty access -> SynthDict ctx <$> visitGeneric ty <*> pure access
...
instance IRRep r => VisitGeneric (TabPiType r) r where
  visitGeneric (TabPiType d b eltTy) = do
    d' <- visitGeneric d
    visitGeneric (PiType (UnaryNest b) Pure eltTy) <&> \case
      PiType (UnaryNest b') Pure eltTy' -> TabPiType d' b' eltTy'
