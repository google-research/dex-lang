...
instance VisitGeneric NewtypeTyCon CoreIR where
  visitGeneric = \case
    ...
    UserADTType n v params repTy -> UserADTType n <$> renameN v <*> visitGeneric params <*> visitType repTy
...
instance IRRep r => VisitGeneric (TabPiType r) r where
  visitGeneric (TabPiType d b (Abs ds eltTy)) = do
    d' <- visitGeneric d
    visitGeneric (PiType (UnaryNest b) (Abs ds (EffTy Pure eltTy))) <&> \case
      PiType (UnaryNest b') (Abs ds' (EffTy Pure eltTy')) -> TabPiType d' b' (Abs ds' eltTy')
      _ -> error "not a table pi type"

instance IRRep r => VisitGeneric (DepPairType r) r where
  visitGeneric (DepPairType expl b (Abs ds ty)) = do
    visitGeneric (PiType (UnaryNest b) (Abs ds (EffTy Pure ty))) <&> \case
      PiType (UnaryNest b') (Abs ds' (EffTy Pure ty')) -> DepPairType expl b' (Abs ds' ty')
