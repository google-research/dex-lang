...
instance SourceRenamableB UDecl where
  sourceRenameB decl cont = case decl of
    ...
    UHandlerDecl effName tyArgs retTy ops handlerName -> do
      effName' <- sourceRenameE effName
      Abs tyArgs' (PairE (ListE ops') retTy') <- sourceRenameE (Abs tyArgs (PairE (ListE ops) retTy))
      sourceRenameUBinder UHandlerVar handlerName \handlerName' -> do
        cont $ UHandlerDecl effName' tyArgs' retTy' ops' handlerName'
...
instance HasSourceNames UDecl where
  sourceNames decl = case decl of
    ...
    UHandlerDecl _ _ _ _ handlerName -> sourceNames handlerName
