...
applyIdxs :: Builder n m => Atom n -> IndexStructure n -> m (Atom n)
applyIdxs = undefined
...
offsetToE :: Builder n m => IndexStructure n -> Atom n -> m (Atom n)
offsetToE idxs i = evalSumClampPolynomial (offsets idxs) i

elemCountE :: Builder n m => IndexStructure n -> m (Atom n)
...
indexSetSize (PrimTC con) = case con of
...
toPolynomial atom = case atom of
  ...
  PrimCon (Lit (Int64Lit x)) -> fromInt x
  PrimCon (Lit (Int32Lit x)) -> fromInt x
  PrimCon (Lit (Word8Lit x)) -> fromInt x
  PrimCon (IntRangeVal _ _ i) -> toPolynomial i
...
_evalClampPolynomial :: Builder n m => ClampPolynomial n -> m (Atom n)
_evalClampPolynomial cp = evalPolynomialP (evalClampMonomial Var) cp

evalSumClampPolynomial :: Builder n m
...
ipow :: Builder n m => Atom n -> Int -> m (Atom n)
...
freeVarsP p = undefined -- foldMap freeVarsM $ keys p
