...
evalLLVM block = do
  ...
  ImpFunctionWithRecon impFun reconAtom <- checkPass ImpPass $ toImpFunction resultTy blockAbs
  (_, llvmAST) <- impToLLVM mainFuncName cc impFun
  let IFunType _ resultTypes = impFunType impFun
