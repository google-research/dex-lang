...
module Inference (inferTopUDecl) where
...
inferTopUDecl :: (Fallible m, TopLogger m) => UTopDecl -> m CTopDecl
inferTopUDecl decl = liftInfererM $ case decl of
...
-- asTopBlock :: EnvReader m => CExpr n -> m n (TopBlock n, CType n)
-- asTopBlock block = do
...
newtype InfererM (i::S) (o::S) (a:: *) = InfererM
  { runInfererM' :: SubstReaderT Name (ReaderT1 InfState (ScopeReaderT (ExceptT (State TypingInfo)))) i o a }
  deriving (Functor, Applicative, Monad, MonadFail, Fallible, ScopeReader, SubstReader Name)
...
liftInfererM :: (TopLogger m, Fallible m) => InfererM VoidS VoidS a -> m a
liftInfererM cont = do
  let (ansExcept, typeInfo) = liftInfererMPure cont
  emitLog $ Outputs [SourceInfo $ SITypingInfo typeInfo]
  liftExcept ansExcept
{-# INLINE liftInfererM #-}

liftInfererMPure :: InfererM VoidS VoidS a -> (Except a, TypingInfo)
liftInfererMPure cont = do
  let ansM = runScopeReaderT emptyOutMap $ runReaderT1 emptyInfState $ runSubstReaderT idSubst $ runInfererM' cont
  runState (runExceptT ansM) mempty
 where
  emptyInfState :: InfState n
  emptyInfState = InfState (Givens HM.empty)
...
-- withInferenceVar
--   :: (Zonkable e, Emits o, ToBinding binding) => NameHint -> binding o
...
checkOrInfer :: RequiredTy o -> UExpr i -> InfererM i o (CAtom o)
checkOrInfer reqTy expr = case reqTy of
  Infer -> bottomUp expr
  Check t -> topDown t expr

topDown :: CType o -> UExpr i -> InfererM i o (CExpr o)
topDown ty uexpr = undefined -- topDownPartial (typeAsPartialType ty) uexpr

topDownPartial :: PartialType o -> UExpr i -> InfererM i o (CAtom o)
topDownPartial partialTy exprWithSrc@(WithSrcE sid expr) = undefined
...
etaExpandPartialPi
  :: PartialPiType o
  -> (forall o'. (DExt o o') => RequiredTy o' -> [CAtom o'] -> InfererM i o' (CAtom o'))
  -> InfererM i o (CoreLamExpr o)
etaExpandPartialPi (PartialPiType appExpl expls bs reqTy) cont = undefined
-- etaExpandPartialPi (PartialPiType appExpl expls bs reqTy) cont = do
--   withFreshBindersInf expls (Abs bs reqTy) \bs' reqTy' -> do
...
--     return $ CoreLamExpr piTy $ Abs bs' body

-- Expects the uexpr to be a type-like thing, including ordinary data types, pi
-- types, dict types etc.
topDownType :: UExpr i -> InfererM i o (CType o)
topDownType exprWithSrc@(WithSrcE sid expr) = undefined
...
topDownExplicit :: forall i o. CType o -> UExpr i -> InfererM i o (CAtom o)
topDownExplicit = undefined
...
fromIntLit :: SrcId -> Int -> CType o -> InfererM i o (CAtom o)
fromIntLit sid x ty = do
  let litVal = CLit $ Int64Lit $ fromIntegral x
...
instantiateSigma :: SrcId -> RequiredTy o -> SigmaAtom o -> InfererM i o (CAtom o)
instantiateSigma sid reqTy sigmaAtom = case sigmaAtom of
  SigmaUVar _ _ _ -> undefined
...
matchReq sid (Check reqTy) x = do
  reqTy' <- sinkM reqTy
  return x <* expectEq sid reqTy' (getCType x)
matchReq _ Infer x = return x
{-# INLINE matchReq #-}

forceSigmaAtom :: SigmaAtom o -> InfererM i o (CAtom o)
forceSigmaAtom sigmaAtom = case sigmaAtom of
  SigmaAtom _ x -> return x
  SigmaUVar _ _ v -> undefined
...
applyFromLiteralMethod :: SrcId -> CType n -> SourceName -> CAtom n -> InfererM i n (CAtom n)
applyFromLiteralMethod sid resultTy methodName litVal = undefined
...
data SigmaAtom n =
    SigmaAtom (Maybe SourceName) (CExpr n)
...
-- instance HasType SigmaAtom where
--   getType = \case
...
class PrettyE e => ExplicitArg (e::E) where
  checkExplicitNonDependentArg :: e i -> PartialType o -> InfererM i o (CAtom o)
  checkExplicitDependentArg    ::            e i -> PartialType o -> InfererM i o (CAtom o)
  inferExplicitArg :: e i -> InfererM i o (CAtom o)
  isHole :: e n -> Bool
...
checkOrInferApp appSrcId funSrcId f' posArgs namedArgs reqTy = undefined
...
-- buildAppConstraints appSrcId reqTy (CorePiType _ _ bs ty) = do
--   buildConstraints (Abs bs ty) \_ resultTy -> do
--     return case reqTy of
--       Infer -> []
...
-- maybeInterpretPunsAsTyCons = undefined
...
-- applySigmaAtom (SigmaAtom _ f) args = emit =<< mkApp f args
...
-- --     emit =<< mkApplyMethod (fromJust $ toMaybeDict dictArg) methodIdx args'
-- applySigmaAtom (SigmaPartialApp _ f prevArgs) args =
--   emit =<< mkApp f (prevArgs ++ args)
...
-- data Constraint (n::S) = TypeConstraint SrcId (CType n) (CType n)
...
checkMaybeAnnExpr ty expr = case ty of
...
-- inferDotMethod
--   :: TyConName o
--   -> Abs (Nest UAnnBinder) (Abs UBinder ULamExpr) i
--   -> InfererM i o (CoreLamExpr o)
...
--   where
--     go :: [CType l] -> [Projection] -> Nest (Binder) l p -> (CType l, [[Projection]])
...
--   PartialPiType piAppExpl expls piBs piReqTy <- return partialPiTy
--   ULamExpr lamBs lamAppExpl lamResultTy body <- return lamExpr
...
--     piReqTy' <- applyRename (piBs @@> (atomVarName <$> bindersVars lamBs')) piReqTy
...
--     body' <- buildBlock $ withBlockDecls body \result -> checkOrInfer (sink resultTy) result
--     resultTy' <- case resultTy of
--       Infer -> return $ getType body'
--       Check t -> return t
--     let piTy = CorePiType piAppExpl expls lamBs' resultTy'
...
-- inferUForExpr :: Emits o => UForExpr i -> InfererM i o (LamExpr o)
...
-- checkUForExpr :: Emits o => SrcId -> UForExpr i -> TabPiType o -> InfererM i o (LamExpr o)
-- checkUForExpr sid (UForExpr bFor body) (TabPiType _ bPi resultTy) = do
--   let uLamExpr = ULamExpr (UnaryNest bFor) ExplicitApp Nothing body
--   partialPi <- liftEnvReaderM $ refreshAbs (Abs bPi resultTy) \bPi' resultTy' -> do
--     return $ PartialPiType ExplicitApp [Explicit] (UnaryNest bPi') (Check resultTy')
--   CoreLamExpr _ lamExpr <- checkULamPartial partialPi sid uLamExpr
--   return lamExpr

-- inferULam :: ULamExpr i -> InfererM i o (CoreLamExpr o)
-- inferULam (ULamExpr bs appExpl resultTy body) = do
--   Abs (ZipB expls bs') (PairE ty body') <- inferUBinders bs \_ -> do
...
--     return $ PairE (getType body') body'
--   return $ CoreLamExpr (CorePiType appExpl expls bs' ty) (LamExpr bs' body')
...
-- piAsPartialPi (CorePiType appExpl expls bs ty) =
--   PartialPiType appExpl expls bs (Check ty)
...
-- piAsPartialPiDropResultTy (CorePiType appExpl expls bs _) =
--   PartialPiType appExpl expls bs Infer
...
--     bindLetPats ps xs cont
--   where
--     emitInline :: Emits n => CAtom  n -> InfererM i n (AtomVar n)
...
-- checkTabCon :: forall i o. Emits o => TabPiType o -> SrcId -> [UExpr i] -> InfererM i o (CAtom o)
...
-- getIxDict :: SrcId -> CType o -> InfererM i o (IxDict o)
-- getIxDict sid t = fromJust <$> toMaybeDict <$> trySynthTerm sid (toType $ IxDictType t) Full

-- asIxType :: SrcId -> CType o -> InfererM i o (IxType o)
...
-- lookupSolverSubst :: forall n. SolverSubst n -> Name n -> AtomSubstVal n
-- lookupSolverSubst (SolverSubst m) name = undefined
...
-- constrainEq :: ToAtom e => SrcId -> e o -> e o -> SolverM i o ()
...
-- instance Unifiable (Stuck) where
--   unify s1 s2 = do
--     x1 <- zonkStuck s1
--     x2 <- zonkStuck s2
--     case (x1, x2) of
...
-- unifyStuckConZonked :: CStuck n -> Con n -> SolverM i n ()
-- unifyStuckConZonked s x = case s of
--   Var v -> extendSolution v (Con x)
--   _ -> empty

-- unifyStuckCon :: CStuck n -> Con n -> SolverM i n ()
...
-- instance Unifiable (TyCon) where
...
--    where
--     go :: Abs (Nest CBinder) (Type) n
--        -> Abs (Nest CBinder) (Type) n
--        -> SolverM i n ()
--     go (Abs Empty t1) (Abs Empty t2) = unify t1 t2
...
-- instance Unifiable (TabPiType) where
--   unify (TabPiType _ b1 ty1) (TabPiType _ b2 ty2) =
--     unify (Abs b1 ty1) (Abs b2 ty2)

-- instance Unifiable (DepPairType) where
...
--                    => (PairE CAtom CAtom n) -> m n (Abs (Nest (AtomNameBinder)) (PairE CAtom CAtom) n)
-- renameForPrinting e = do
--   infVars <- filterM isSolverName $ freeVarsList e
...
-- emitInstanceDef :: (Mut n, TopBuilder m) => InstanceDef n -> m n (Name n)
...
-- hasInferenceVars e = liftEnvReaderM $ anyInferenceVars $ freeVarsList e
...
--   TyCon (Pi (CorePiType ImplicitApp expls bs (TyCon (DictTy d)))) ->
...
-- synthTerm sid targetTy reqMethodAccess = case targetTy of
...
--     let piTy = CorePiType ImplicitApp expls bs' (getType synthExpr)
...
--     CorePiType _ expls bs (TyCon (DictTy candidateTy)) <- lookupInstanceTy candidate
...
--   -> InfererM i n (PairE CExpr (ReconAbs e) n)
...
-- checkFFIFunTypeM (CorePiType appExpl (_:expls) (Nest b bs) ty) = do
--   argTy <- checkScalar $ binderType b
--   case bs of
--     Empty -> do
--       resultTys <- checkScalarOrPairType ty
...
--     Nest b' rest -> do
--       let naryPiRest = CorePiType appExpl expls (Nest b' rest) ty
...
--   substE env (SigmaUVar sn ty uvar) = undefined
...
--   type RepE Constraint = PairE (LiftE SrcId) (PairE CType CType)
--   fromE (TypeConstraint   sid t1 t2) = LiftE sid `PairE` PairE t1 t2
--   {-# INLINE fromE #-}
--   toE (LiftE sid `PairE` PairE t1 t2) = TypeConstraint   sid t1 t2
--   {-# INLINE toE #-}
...
--   type RepE PartialPiType = LiftE (AppExplicitness, [Explicitness]) `PairE` Abs (Nest CBinder) RequiredTy
--   fromE (PartialPiType ex exs b ty) = LiftE (ex, exs) `PairE` Abs b ty
--   {-# INLINE fromE #-}
--   toE   (LiftE (ex, exs) `PairE` Abs b ty) = PartialPiType ex exs b ty
--   {-# INLINE toE #-}
...
getUVarType :: UVar o -> InfererM i o (CType o)
