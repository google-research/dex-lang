...
linearizeAtom atom = case atom of
  ...
  ProjectElt idxs v -> do
    ...
    x' <- return $ getProjection (toList idxs) x
    return $ WithTangent x' do
      t <- tx
      return $ getProjection (toList idxs) t
...
applyCustomLinearization :: Emits o => AtomRules o -> [SAtom i] -> LinM i o (Atom CS) (Atom CS)
applyCustomLinearization _ _ = undefined
-- applyCustomLinearization (CustomLinearize n zeros cl) xs = do
--   let (polyXs, argXs) = splitAt n $ toList xs
--   polyXs' <- mapM (substM . injectCS) polyXs
...
--             return $ WithTangent (injectCS arg') $
--               return $ sink $ Con $ Newtype
--                 (TypeCon "SymbolicTangent" stDefName
--                  (DataDefParams [(PlainArrow, injectCS argTy')]))
--                 (SumVal [UnitTy, injectCS argTy'] 0 UnitVal)
...
--                  (DataDefParams [(PlainArrow, sink (injectCS argTy'))]))
--                 (SumVal [UnitTy, sink (injectCS argTy')] 1 argTan)
