...
import Prelude hiding ((.), id, pi, abs)
  ...
  Case e alts _ -> do
    ...
    case e' of
      DataCon _ _ con args -> do
        case alts !! con of
          Abs bs body -> translateBlock (env >>> bs @@> map SubstVal args) (maybeDest, body)
      Variant (NoExt types) label i x -> do
        let LabeledItems ixtypes = enumerate types
        let index = fst $ ixtypes M.! label NE.!! i
        case alts !! index of
          Abs bs body -> translateBlock (env >>> bs @@> [SubstVal x]) (maybeDest, body)
      Con (SumAsProd _ tag xss) -> do
        ...
        emitSwitch tag' $ flip map (zip xss alts) $
          \(xs, Abs bs body) ->
             void $ translateBlock (env >>> bs @@> map SubstVal xs) (Just dest, body)
        destToAtom dest
  ...
  case hof of
    ...
    While (Lam (Abs b (WithArrow _ body))) -> do
      body' <- liftM snd $ scopedBlock $ do
                 ans <- translateBlock (env >>> b @> SubstVal UnitVal) (Nothing, body)
    ...
    RunReader r (BinaryFunVal h ref _ body) -> do
      ...
      copyAtom rDest r'
      let env' = env >>> h @> undefined >>> ref @> SubstVal rDest
    ...
    RunState s (BinaryFunVal h ref _ body) -> do
      ...
      let env' = env >>> h @> undefined >>> ref @> SubstVal sDest
      void $ translateBlock env' (Just aDest, body)
      PairVal <$> destToAtom aDest <*> destToAtom sDest
    RunIO (Lam (Abs b (WithArrow _ body))) ->
      translateBlock (env >>> b @> SubstVal UnitVal) (maybeDest, body)
