...
getNaryLamImpArgTypes t = liftEnvReaderM $ go t where
  ...
  go (PiType (WithAttrs _ bs) effs resultTy) = case bs of
    Nest piB rest -> do
      ts <- getRepBaseTypes $ binderType piB
      refreshAbs (Abs piB (PiType (WithAttrs PlainArgs rest) effs resultTy)) \_ restPi -> do
...
interpretImpArgs t xsAll = liftEnvReaderM $ runSubstReaderT idSubst $ go t xsAll where
  ...
  go (PiType (WithAttrs _ bs) effs resultTy) xs = case bs of
    Nest (b:>argTy) rest -> do
      argTy' <- substM argTy
      (argTree, xsRest) <- listToTree argTy' xs
      arg <- repValAtom $ RepVal argTy' argTree
      (args, dest) <- extendSubst (b @> SubstVal arg) $
        go (PiType (WithAttrs PlainArgs rest) effs resultTy) xsRest
