...
peepholeOp op = case op of
  ...
  MiscOp (ToEnum ty (Con (Lit (Word8Lit tag)))) -> case ty of
    SumTy cases -> return $ Con $ SumCon cases (fromIntegral tag) (toExpr UnitVal)
...
newtype ULM i o a = ULM { runULM :: SubstReaderT ExprSubstVal (StateT1 ULS (BuilderM SimpIR)) i o a}
  deriving (MonadFail, Fallible, Functor, Applicative, Monad, ScopeReader,
            EnvReader, EnvExtender, Builder SimpIR, SubstReader ExprSubstVal,
...
instance Visitor (ULM i o) SimpIR i o where
  visitExpr = undefined -- visitAtomDefault
...
instance Visitor (LICMM i o) SimpIR i o where
  visitExpr = undefined -- visitExprDefaultNoEmits
  visitType = visitTypeDefault
  visitPi   = visitPiDefault
  visitLam  = visitLamEmits
...
hoistLoopInvariant = undefined -- liftLamExpr hoistLoopInvariantBlock
...
--       (oldIx, allCarries) <- fromPair $ AVar $ binderVar lb'
...
seqLICM :: RNest SDecl n1 n2      -- hoisted decls
        ...
        -> SubstReaderT ExprSubstVal EnvReaderM m1 n4
...
dceTop = undefined -- liftLamExpr dceBlock
{-# INLINE dceTop #-}
...
instance HasDCE (LamExpr SimpIR) where
  dce (LamExpr bs e) = undefined -- dceBinders bs e \bs' e' -> LamExpr bs' <$> dceBlock' e'
